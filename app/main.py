...
def _validate_security_config() -> None:
    if not settings.inbound_signature_secret:
    ...
    if settings.environment.lower() in {"staging", "production", "prod"}:
        if settings.inbound_signature_secret == settings.inbound_signature_dev_fallback:
            raise RuntimeError("INBOUND_SIGNATURE_SECRET cannot use the development fallback in staging/production")
        if settings.outbound_signature_secret == settings.outbound_signature_dev_fallback:
