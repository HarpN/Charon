param(
    ...
    [string]$ReleaseName = "charon",
    [string]$ImageRepository = "charon-agent",
    [string]$ImageTag = "latest",
    [string]$JudyNamespace = "governance-zone"
)
...
Write-Host "[5/6] Applying Charon Helm release '$ReleaseName'..."
helm upgrade --install $ReleaseName ./charts/charon `
    --namespace $Namespace `
    --create-namespace `
    --set image.repository=$ImageRepository `
    --set image.tag=$ImageTag `
    --set networkPolicy.judyNamespace=$JudyNamespace

Write-Host "[6/6] Current rollout status..."
kubectl rollout status deployment/$ReleaseName-charon -n $Namespace --timeout=120s
