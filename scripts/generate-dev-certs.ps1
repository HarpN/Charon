...
$charonDir = Join-Path $certsRoot "charon"
...
New-Item -ItemType Directory -Path $charonDir -Force | Out-Null
...
$serverCaKey = Join-Path $caDir "charon-server-ca.key"
$serverCaCert = Join-Path $caDir "charon-server-ca.crt"

New-Ca -CommonName "Charon Dev Clients CA" -KeyPath $clientsCaKey -CertPath $clientsCaCert -Days $DaysValid
New-Ca -CommonName "Judy Dev Server CA" -KeyPath $judyCaKey -CertPath $judyCaCert -Days $DaysValid
New-Ca -CommonName "Charon Dev Server CA" -KeyPath $serverCaKey -CertPath $serverCaCert -Days $DaysValid

$charonServerExt = Join-Path $tmpDir "charon-server.cnf"
$charonServerCertParams = @{
    CommonName = "charon"
    KeyPath = (Join-Path $charonDir "server.key")
    CsrPath = (Join-Path $tmpDir "charon-server.csr")
    CertPath = (Join-Path $charonDir "server.crt")
    CaKeyPath = $serverCaKey
    CaCertPath = $serverCaCert
    ExtFilePath = $charonServerExt
    ExtendedKeyUsage = "serverAuth"
    Sans = @("charon", "localhost", "host.docker.internal")
    Days = $DaysValid
}
New-SignedCert @charonServerCertParams

$charonClientExt = Join-Path $tmpDir "charon-client.cnf"
$charonClientCertParams = @{
    CommonName = "charon-client"
    KeyPath = (Join-Path $charonDir "client.key")
    CsrPath = (Join-Path $tmpDir "charon-client.csr")
    CertPath = (Join-Path $charonDir "client.crt")
    CaKeyPath = $judyCaKey
    CaCertPath = $judyCaCert
    ExtFilePath = $charonClientExt
    ExtendedKeyUsage = "clientAuth"
    Sans = @("charon-client")
    Days = $DaysValid
}
New-SignedCert @charonClientCertParams
...
Write-Host "Use docker-compose.mtls.yml to run Charon in mTLS mode."
